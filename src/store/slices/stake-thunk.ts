...
export const changeApproval = createAsyncThunk("stake/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const gobContract = new ethers.Contract(addresses.GOB_ADDRESS, TimeTokenContract, signer);
    const sgobContract = new ethers.Contract(addresses.SGOB_ADDRESS, MemoTokenContract, signer);
    ...
    try {
        ...
        if (token === "gob") {
            approveTx = await gobContract.approve(addresses.STAKING_HELPER_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
        }

        if (token === "sgob") {
            approveTx = await sgobContract.approve(addresses.STAKING_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
        }

        const text = token === "gob" ? i18n.t("stake:ApproveStaking") : i18n.t("stake:ApproveUnstaking");
        const pendingTxnType = token === "gob" ? "approve_staking" : "approve_unstaking";
    ...
    const stakeAllowance = await gobContract.allowance(address, addresses.STAKING_HELPER_ADDRESS);
    const unstakeAllowance = await sgobContract.allowance(address, addresses.STAKING_ADDRESS);

    return dispatch(
        fetchAccountSuccess({
            staking: {
                sgobStake: Number(stakeAllowance),
                sgobUnstake: Number(unstakeAllowance),
            },
        }),
    );
});
